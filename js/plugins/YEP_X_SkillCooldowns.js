...
//=============================================================================
 /*:
 * @plugindesc v1.11mod (Requires YEP_SkillCore.js) Cooldowns can be applied
 ...
 * @default 75
 *
 * @param Cooldown Cost Format
 * @desc This is the text format used for cooldown costs. Leave empty to hide.
 * %1 - Cooldown Length
 * @default  
 *
 * @param Cooldown Cost Icon
 * @desc What icon to be used for cooldown costs.
 * Use 0 for no icon.
 * @default 0
 *
...
Yanfly.Param.CDCostFmt = String(Yanfly.Parameters['Cooldown Cost Format']);
Yanfly.Icon.CooldownCost = Number(Yanfly.Parameters['Cooldown Cost Icon']);
...
Yanfly.SCD.Window_SkillList_dOC = Window_SkillList.prototype.drawOtherCost;
Window_SkillList.prototype.drawOtherCost = function(skill, wx, wy, dw) {
    dw = this.drawCooldownCost(skill, wx, wy, dw);
    return Yanfly.SCD.Window_SkillList_dOC.call(this, skill, wx, wy, dw);
};

Window_SkillList.prototype.drawCooldownCost = function(skill, wx, wy, dw) {
    var cooldown = 0;
    for (var skillId in skill.cooldown) {
        skillId = parseInt(skillId);
        if (!$dataSkills[skillId]) continue;
        cooldown = skill.cooldown[skillId];
    }
    if (cooldown <= 0) return dw;
    if (cooldown % 1 !== 0) cooldown = cooldown.toFixed(2);
    if (cooldown <= 0.009) cooldown = 0.01;
    if (Yanfly.Param.CDCostFmt === '') return dw;
    if (Yanfly.Icon.CooldownCost > 0) {
        var iw = wx + dw - Window_Base._iconWidth;
        this.drawIcon(Yanfly.Icon.CooldownCost, iw, wy + 2);
        dw -= Window_Base._iconWidth + 2;
    }
    this.contents.fontSize = Yanfly.Param.CDFontSize;
    this.changeTextColor(this.textColor(Yanfly.Param.CDTextColor));
    var fmt = Yanfly.Param.CDCostFmt;
    var text = fmt.format(Yanfly.Util.toGroup(cooldown));
    this.drawText(text, wx, wy, dw, 'right');
    var returnWidth = dw - this.textWidth(text) - Yanfly.Param.SCCCostPadding;
    this.resetTextColor();
    this.resetFontSettings();
